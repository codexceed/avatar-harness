...
`DeclarationRequired`, which the transcript shows as `✍ declare a verification contract before
editing` while the model declares one via `declare_verification`. This is observe-only — no modal,
