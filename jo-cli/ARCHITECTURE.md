...
| `app.py` (`CockpitApp`) | The shell: status bar + `RichLog` transcript + a `HistoryInput` (an `Input` with ↑/↓ recall of the sitting's submitted prompts). Two modes — **observe** a fixed `session=` stream, or **drive** a live `repl=` `ReplSession`. Renders `events()`; acts via modals. `ctrl+c` copies an active text selection if there is one, else **hard-cancels** the in-flight run (`_run_task.cancel()` — instant, aborting an in-flight model call at the socket via the async client, ADR-0024; `_observe` marks it cancelled), else quits (a finished/cancelled goal clears the per-goal run so it always falls through to quit). An external `SIGINT`/`SIGTERM` is handled (skipped in headless tests) for a graceful shutdown. |
