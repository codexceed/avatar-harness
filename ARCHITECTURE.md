...
| `SessionState` + `ReplSession` | The multi-turn scope above `TaskState` (`session_state.py`): history, per-goal tasks, session-scoped grants, mode; drives each goal through a per-goal `Session`. | [Implemented] (Lane 2a; Textual cockpit that renders it = Lane 2b/2c [Designed]) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): Lane 2b/2c — Textual cockpit shell + approval/plan/diff modals (rendering the [Implemented] Lane 2a `SessionState`/`ReplSession`); tail — plan mode + conversational-verification authority; 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions. (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2a — multi-turn `SessionState`/`ReplSession`, and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
