...
Remaining **[Designed]** (per **ADR-0001/0002**): Lane 1 — bounded `AsyncEventBus` + privileged write-ahead journal; Lane 2 — Textual cockpit + multi-turn `SessionState`/`submit()`/visible-mode routing; Lane 3 — prefix-scoped approval grants (the `run_command` tool is [Implemented]); tail — plan mode + conversational-verification authority; 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions.
