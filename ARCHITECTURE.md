...
| `tui/` (cockpit) | `CockpitApp` (`tui/app.py`) renders `session.events()` into a status bar + transcript + input; `ReplaySession` (`tui/replay.py`) replays a fixed event list for deterministic tests/`--replay`; behind the optional `[textual]` extra (`load_cockpit()` guard). | [Implemented] (Lane 2b shell; approval/plan/diff modals = Lane 2c [Designed]) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): Lane 2c — approval/plan/diff modals over the [Implemented] cockpit shell; tail — plan mode + conversational-verification authority + meta commands/`@path`; 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification, the CLI flag that launches the cockpit, and §21 extensions. (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2a — `SessionState`/`ReplSession`, Lane 2b — Textual cockpit shell, and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
