...
| `SessionState` + `ReplSession` | The multi-turn scope above `TaskState` (`session_state.py`): history, per-goal tasks, session-scoped grants, mode; drives each goal through a per-goal `Session`; handles local meta commands (`run_meta` → `MetaResult`) that never reach the model; seeds `@path` references as denylist-checked `grounding` evidence. | [Implemented] (Lane 2a + 3.2a meta commands + 3.2b `@path`) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): the rest of the 3.2 tail — plan-mode flow (3.2c), conversational-verification authority (3.2d), the `--interactive` CLI flag that launches the cockpit (3.2e); 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions. (**3.2a meta commands + 3.2b `@path` grounding are [Implemented]**.) (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2 — `SessionState`/`ReplSession` + the Textual cockpit (shell + modals), and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
