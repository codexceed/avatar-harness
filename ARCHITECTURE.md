...
>
> **Phase 3.0 (foundation) is now [Implemented]:** the engine has an **async core** — `AgentRunner.arun()` is the loop, sync `run()` wraps it via `asyncio.run()`, blocking bodies offloaded with `to_thread`; a **typed `HarnessEvent` union** (`event_types.py`, closed/versioned/discriminated) published through an **`EventBus`** that fans out to independent per-subscriber streams; and the **two-plane `Session`** — observation out via `events()` (cannot block/redirect), control in via `resolve_approval()` / `cancel()`. The facade exposes the async surface (`Harness.arun()` / `Harness.session()`), and the typed events + `Session` are exported from the package. This is a **foundation**, not a sealed SDK: **Lanes 1 and 3 are now [Implemented]** — Lane 1 is the bounded `EventBus` (`bus.py`; soft-cap per-subscriber queues that shed only droppable `*_update` events, lifecycle/control never dropped) + the privileged write-ahead `JsonlEventJournal` (`journal.py`; lossless, flushed per event); Lane 3 is the tier-3 `run_command` tool *and* the prefix-scoped, session-scoped `ApprovalGrant` (`[a] always` via `resolve_approval(remember=True)`; the grant lives in `Session`, not the gate). **Lane 2 (the cockpit) is [Implemented]** — 2a is the multi-turn `SessionState` + `ReplSession` driver (`session_state.py`): each goal runs as a fresh `TaskState` through the single-task `Session` (one code path), history seeds the next task, grants persist across tasks, a visible heuristic routes the mode. 2b/2c are the Textual cockpit (`tui/`): `CockpitApp` renders `session.events()` into a status bar + transcript + input and auto-pops the `ApprovalModal` (→ `resolve_approval`) on a gated call; `DiffModal`/`PlanModal` round out the surfaces; all tested headlessly against a `ReplaySession`, behind the optional `[textual]` extra. **The 3.2 tail is [Implemented] through 3.2d** — meta commands (3.2a), `@path` grounding (3.2b), **plan mode (3.2c)** (a read-only plan task → human approve/revise → the approved plan seeds the edit task as a constraint, riding the existing phase gate; `ReplSession.submit_plan(prompt, decide)` drives it, decision via a Textual-free `PlanDecision`), and **conversational-verification authority (3.2d)** (the verifier always runs + reports; a `conversational` flag on `AgentRunner` delivers the reply as advisory without the §12 repair gate — the `ReplSession` default, with `--auto` restoring strict). Still **[Designed]** are the CLI flag that launches the cockpit (3.2e) and durable resume (3.3) — per **ADR-0001/0002**. One live boundary: intake does not yet classify a free-text goal into a `task_kind` (the CLI runs `investigate` by default or takes an explicit kind).
...
| `SessionState` + `ReplSession` | The multi-turn scope above `TaskState` (`session_state.py`): history, per-goal tasks, session-scoped grants, mode; drives each goal through a per-goal `Session`; handles local meta commands (`run_meta` → `MetaResult`) that never reach the model; seeds `@path` references as denylist-checked `grounding` evidence; drives plan mode (`submit_plan`: read-only plan → approve/revise → approved plan seeds the edit task as a constraint). | [Implemented] (Lane 2a + 3.2a meta commands + 3.2b `@path` + 3.2c plan mode) |
| `tui/` (cockpit) | `CockpitApp` (`tui/app.py`) renders `session.events()` into a status bar + transcript + input and auto-pops the approval modal on a gated call; `tui/modals.py` has `ApprovalModal`/`DiffModal`/`PlanModal`; `ReplaySession` (`tui/replay.py`) replays a fixed event list for deterministic tests/`--replay`; behind the optional `[textual]` extra (`load_cockpit()` guard). | [Implemented] (Lane 2b shell + 2c modals) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): the rest of the 3.2 tail — the `--interactive` CLI flag that launches the cockpit (3.2e); 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions. (**3.2a meta commands + 3.2b `@path` grounding + 3.2c plan mode + 3.2d conversational-verification authority are [Implemented]**.) (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2 — `SessionState`/`ReplSession` + the Textual cockpit (shell + modals), and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
