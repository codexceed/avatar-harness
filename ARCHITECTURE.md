...
| `SessionState` + `ReplSession` | The multi-turn scope above `TaskState` (`session_state.py`): history, per-goal tasks, session-scoped grants, mode; drives each goal through a per-goal `Session`. | [Implemented] (Lane 2a) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): the 3.2 tail — plan-mode flow + conversational-verification authority + meta commands/`@path`; 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification, the CLI flag that launches the cockpit, and §21 extensions. (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2 — `SessionState`/`ReplSession` + the Textual cockpit (shell + modals), and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
