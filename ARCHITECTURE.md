...
| `SessionState` + `ReplSession` | The multi-turn scope above `TaskState` (`session_state.py`): history, per-goal tasks, session-scoped grants, mode; drives each goal through a per-goal `Session`; handles local meta commands (`run_meta` → `MetaResult`) that never reach the model. | [Implemented] (Lane 2a + 3.2a meta commands) |
...
Remaining **[Designed]** (per **ADR-0001/0002**): the rest of the 3.2 tail — `@path` grounding + plan-mode flow (3.2b), conversational-verification authority + the CLI flag that launches the cockpit (3.2c); 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions. (**3.2a meta commands are [Implemented]**.) (**Lane 1 — bounded `EventBus` + write-ahead `JsonlEventJournal`, Lane 2 — `SessionState`/`ReplSession` + the Textual cockpit (shell + modals), and Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — are [Implemented]**.)
