...
| `session.py` | `Session` (two-plane: `events()` out · `resolve_approval(remember=)`/`cancel()` in) + `EventBus` (per-subscriber fan-out) + `ApprovalGrant` (session-scoped `[a] always`; the gate stays harness-owned, the Session answers from a remembered grant) | `§13`, `§23` |
...
| `__init__.py` | Curated public API (`__all__`): core (`Harness`/`TaskState`/`Workspace`/`RunDeps`), decisions, tools, **the two-plane async surface** (`Session`/`EventBus`/`EventSink`/`ApprovalController`/`ApprovalGrant`) and the typed `HarnessEvent`s | public surface |
...
Remaining **[Designed]** (per **ADR-0001/0002**): Lane 1 — bounded `AsyncEventBus` + privileged write-ahead journal; Lane 2 — Textual cockpit + multi-turn `SessionState`/`submit()`/visible-mode routing; tail — plan mode + conversational-verification authority; 3.3 — durable resume; plus free-text-goal → `task_kind` intake classification and §21 extensions. (**Lane 3 — `run_command` + prefix-scoped `ApprovalGrant` — is [Implemented]**.)
