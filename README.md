...
AVATAR_YOLO=0                                    # "YOLO": unattended runs auto-approve arbitrary run_command (default off) — see the autonomy note below
```
...
**Fully-autonomous runs (`AVATAR_YOLO`).** `run_command` (arbitrary shell) is tier-3: attended runs ask the human, and **unattended runs deny it by default** — so a batch/eval agent can't run arbitrary commands (it runs the configured `AVATAR_TEST_COMMAND`/`AVATAR_LINT_COMMAND` via the ungated `run_tests`/`run_linter` instead). To let an unattended run execute arbitrary commands on its own, set `AVATAR_YOLO=1` (ADR-0050): it auto-approves `run_command` only — not contract amendments, and only when unattended. Commands still run sandboxed (ADR-0042), argv-only (no shell metacharacters, ADR-0045), and workspace-confined, but this is genuine autonomous code execution — `python -c "…"` can do anything Python can. Opt in deliberately.
