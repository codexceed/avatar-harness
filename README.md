...
- **Approval prompts** for `run_command` and sensitive-path calls (`[y]` once · `[a]` always this session · `[d]` deny); `apply_patch`/`write_file` auto-allow when their paths validate inside the workspace.
