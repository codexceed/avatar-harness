...
```python
Session(runner: AgentRunner, state: TaskState, *, session_id: str | None = None, journal: JsonlEventJournal | None = None, grants: list[ApprovalGrant] | None = None, unattended: bool = False, approval_timeout: float | None = None) -> None
```
