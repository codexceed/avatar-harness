...
```python
ContextBuilder(detail_char_budget: int = 48000, max_detail_chars: int = 16000, max_evidence_lines: int = 40, max_actions: int = 25) -> None
```
...
```python
ContextPacket(*, goal: str, constraints: list[str] = <factory>, phase: str, task_kind: str = 'investigate', plan: list[str] = <factory>, files_read: list[str] = <factory>, files_modified: list[str] = <factory>, prior_actions: list[str] = <factory>, recent_evidence: list[str] = <factory>, allowed_tools: list[ToolSummary] = <factory>, latest_error: str | None = None, has_uncommitted_changes: bool = False) -> None
```
