...
2. **Per-kind coverage at declaration time.** For **each** declared kind, **at least one** check must satisfy that kind's rulebook (coverage per kind — *not* "every check satisfies every kind", which is incoherent):
   ...
   - **`content`** — **anchored + falsifiable** replaces "must run what you build": the check must be able to exit non-zero on a wrong artifact (no pure-emitter-only lines — `true`, `echo`, `printf` —, no `|| true`-style neutralization) and must inspect *something* (an assertive inspector — `test`, `grep`, `cmp`, `diff`, … — with an operand, or a real executor). Inspectors flip from blocklisted to first-class: for text they are the verification.
...
- `VerificationPlanFrozen` gains a field (schema-additive); journals written before this ADR read back with the `["code"]` default.
