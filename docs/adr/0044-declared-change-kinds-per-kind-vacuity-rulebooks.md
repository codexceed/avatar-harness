...
1. **The model declares `change_kinds: list` alongside its contract.** `declare_verification` / `alter_verification` gain a `change_kinds` field — a list of the kinds of change the contract must validate: `"code"` (functional — behavior lives in executable artifacts) and `"content"` (non-functional textual artifacts — docs, specs). A *list*, not a scalar: a mixed change (feature + README) declares both, and each kind demands its own coverage. Omitted → `["code"]` (backward compatible, fails toward strictness). An empty list → model-correctable handler rejection.

   *Typing (refined in review):* the kinds are one shared `ChangeKind = Literal["code", "content"]` (`planner.py`), keying the rulebook registry, typing both tool-input fields, and typing the path classifier's return — so a typo'd kind anywhere in harness code fails the type checker, and the tool's JSON schema advertises the enum to the model up front (fewer burned turns; strict tool-calling transports can even enforce it at generation). An unknown kind is therefore rejected at `ToolRuntime` input validation with pydantic's permitted-values error (§10 model-correctable — same lane as a missing arg), not by the handler. Static typing can't force the registry to be *exhaustive* (dict keys may be a subset of the annotated type), so a test pins `set(get_args(ChangeKind)) == set(CHANGE_KIND_COVERAGE)`. The **journaled fields stay `list[str]`** (`TaskState.declared_change_kinds`, `VerificationPlanFrozen.change_kinds`): journals are durable and read across version skew — a kind added later must not make an older harness unable to parse a newer journal. Tool inputs are transient (writer and reader are the same in-process version), so strictness there is pure win.
...
3. **The diff audits the declaration at verification time.** Self-declared kinds without reconciliation would be self-certification one level up (declare `content`, ship code, dodge execution checks). So when a declared contract froze, the verifier classifies the changed paths and requires **`kinds(diff) ⊆ declared change_kinds`** — an undeclared kind present in the diff fails a required `change_kind_coverage` check, legibly, naming the kind and the offending paths. Path classification fails toward strictness: `.md`/`.rst`/`.txt`/`.adoc` → `content`; everything else — including behavior-bearing config (`pyproject.toml`, CI yaml) — → `code`. *(Refined per PR #112 review: behavior-bearing `.txt` basenames — `requirements*.txt`, `constraints*.txt`, `CMakeLists.txt` — classify as `code` despite the suffix, in both the path classifier and the content anchor; and the anchor must be an argv operand of an asserting stage, so a filename inside a string literal — `python -c "print('README.md')"` — anchors nothing.)* Only *under*-declaration is policed; over-declaring is self-inflicted strictness the model can fix via the gated `alter_verification`.
