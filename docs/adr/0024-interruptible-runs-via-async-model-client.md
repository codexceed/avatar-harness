...
2. **`OpenAIModelClient` implements a natively-async `adecide`** backed by `AsyncOpenAI` (`await client.chat.completions.create(...)`), sharing the existing parse/retry/usage-tally logic with the sync path through a small transport seam (only the call site differs). When its run task is cancelled, the in-flight request is aborted cleanly — no lingering thread.
