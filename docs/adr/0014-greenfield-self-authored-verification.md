...
flowchart TD
  ...
  C -- yes --> S["tier 4: model authors smoke check<br/>provenance = model-smoke<br/>resolved at verify time"]
  C -- no --> N[neutral: nothing to verify]
  S --> V
```
...
Mechanically: when the verifier is reached with an **empty** frozen plan and `files_modified` is non-empty, the harness makes one forced-tool call (the existing `_PROPOSE_TOOL` pattern) for a single smoke command, records it as `PlannedCheck(kind="smoke", provenance="model-smoke")`, journals it, and runs it. The verifier gains no judgment logic — only a new *source*.
...
- **`--auto`** — a self-authored check gating an autonomous run is a *bounded* weakening, but "greenfield can never pass autonomously" is worse. Lowest precedence + journaled provenance keeps it legible. Surface, don't suppress.

### When the floor doesn't apply

Empty plan **and** nothing smoke-testable (docs-only, non-code, a stack the model declines) → fall through to a **neutral "nothing to verify"** (ADR-0008's territory), not a hard failure. Floor and neutral state compose.
...
- **Bounded self-grading (the cost, plainly).** The model picks *what* to smoke-test, so it can pick a check it knows passes (`import main` proves import, not correctness). Intended — it's a floor, not a gate. Bounded by: lowest precedence (any real contract displaces it), provenance-tagged for human + eval visibility, and *executable* (unforgeable), never an assertion. Does **not** reopen §5 for non-greenfield repos.
...
- `PlannedCheck.kind` gains `smoke`; `provenance="model-smoke"`. `TaskState` gains one sanctioned mutation that late-binds the floor **only over an empty frozen plan**.
- Resolver `VerificationPlanner.propose_smoke_check(ws, files_modified)` reuses the `_PROPOSE_TOOL` forced-call pattern on `config.model` (the main model — the floor must work with zero extra config). Any endpoint failure degrades to "no floor" (the neutral disposition), never blocks.
- Prompt the model toward **dependency-free** checks (parse / compile / `--check`, not `import` that needs an uninstalled dep) — greenfield deps usually aren't installed in the workspace, so an import-level check fails for the wrong reason. The model knowing the stack is what makes this language-agnostic *and* robust.
- Runner: in `_averify`, when the frozen plan is empty and `files_modified` is non-empty, resolve + journal the smoke check before `verifier.verify`. Tiers 1–3 freeze untouched. Reject obviously-vacuous commands (`true`, `:`, bare `echo`).
- Verifier: a `smoke` check is positive signal (its `name` is already in the plan's positive set); no other change.
