...
- **Related:** [ADR-0007](0007-dynamic-verification-plan-resolution.md) (tiers 1–3 + freeze-before-editing + no invented rubric); [ADR-0038](0038-scoped-autonomous-amendment-disposition.md) (how amendments are gated); [ADR-0039](0039-held-out-verified-vs-self-reported-success.md) (how evals grade this honestly); `HARNESS_DESIGN.md` §5 (no self-certification), §12 (verification contracts); [ADR-0011](0011-verifier-integrity-under-self-improvement.md) (this widens that threat surface).
...
| --- | --- | --- |
| **definition** of "done" | the model *proposes*; a **gate** ratifies | declare / amend the contract (this ADR + ADR-0038) |
...
| Freeze the declared contract hard (no amendment) | Rejected for the attended case — legitimately-obsolete early checks then strand the run; the gated amendment (ADR-0038) handles obsolescence without silent goalpost-moving. Retained as the *unattended default* (deny). |
...
- Greenfield `edit` runs get a **behavioral** contract, not just a compile floor — stronger verification, and (with ADR-0039) an honest eval signal.
...
- **ADR-0011 interaction deepens.** A model that authors *and* petitions to change its contract is a larger self-improvement threat surface. The mitigations: the immutable floor, the amendment gate (ADR-0038), and — decisively for measurement — the held-out oracle (ADR-0039) that grades against a check the model never sees.
- **Mandatory-for-greenfield** raises the floor of what greenfield verification *is*, but the decline-fallback (smoke floor alone) means a model that declares nothing still gets graded, never stuck at declaration.
