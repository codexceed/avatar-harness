...
- Residual: planner-proposed checks already split per segment, but segments containing `|` (pipes inside a CI line) would still mangle at execution; the planner path is repo-derived, not model-authored, and is left for a follow-up if dogfood surfaces it.
