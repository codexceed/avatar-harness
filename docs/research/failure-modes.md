...
### A9 · A NUL/hung provider reply truncates the run 🔧 · 📋 fix proposed (ADR-0028)
...
- **Fix:** ADR-0028 + `CP-transport-retry-nul-resilience` — bound every request (`request_timeout_seconds=90` ≪ wall clock); classify empty/NUL bodies as a new `EmptyResponseError` (transport, not parse); retry transport with **backoff + jitter** (`transport_max_retries=3`) re-issuing the same `messages`; on exhaustion surface a **system failure** (§16) and emit a `transport_retry` event. Pairs with an eval-runner jitter / per-provider concurrency-cap change that removes the trigger.
