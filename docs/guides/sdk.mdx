...
| `AVATAR_REQUEST_TIMEOUT_SECONDS` | `90` | Per-request timeout for one model call (ADR-0028). Bounds a single `chat.completions.create` so a hung/stalled provider can't consume the whole run; must stay well under `AVATAR_MAX_WALL_CLOCK_SECONDS`. |
| `AVATAR_TRANSPORT_MAX_RETRIES` | `3` | Transport-layer retries on a NUL/empty body or a request failure (ADR-0028): the same request is re-issued with exponential backoff + jitter — never re-prompted to the model. On exhaustion the run surfaces a system failure, never a silent `incomplete`. |
