...
Everything documented here is exported from `avatar_harness.__all__` — the
...
| `harness.session(task, *, task_kind, allow_dirty, journal)` | a not-yet-started `Session` | you want the event stream and the approval/cancel controls. |
...
```python
import asyncio
from avatar_harness import ApprovalRequested, Harness
...
<Note>
  ...
  sensitive-path denylist hits. `apply_patch` is tier 1 — it auto-allows once
  its target paths validate inside the workspace, and the verifier judges the
  resulting diff.
</Note>
...
| `ApprovalResolved` | `approval_id`, `allowed`, `via` | The decision (`via="human"` or `"grant"`). |
...
`__all__` (deep-import from `avatar_harness.event_types` for now): `ModelUsage`
...
```python
from avatar_harness import Harness, ReplSession
...
- **Plan mode** — `repl.submit_plan(prompt, decide)` drives read-only plan →
...
The bundled Textual cockpit (`avatar-harness --interactive`) is exactly this
surface rendered full-screen — nothing it does is unavailable to your own UI.
...
| `AVATAR_TEST_COMMAND` | `pytest -q` | The verifier runs this itself — never model-mediated. Empty disables. |
| `AVATAR_LINT_COMMAND` | `ruff check` | Verifier-owned lint/type command. Empty disables. |
...
harness = Harness(
    config=HarnessConfig(workspace_root=str(repo)),
    model=ScriptedModel([
        ...
        ModelDecision(thought_summary="answer",
                      action=FinalAnswer(answer="calc.py defines add(); see lines 1-2.")),
    ]),
)
state = harness.run("what does calc.py do?")   # runs the real loop, verifier and all
```
