...
The repo is a uv workspace; the SDK is the `avatar-harness/` member (import `avatar`).
...
pip install -e './avatar-harness[openai,textual]'   # + the bundled Textual cockpit, `jo-cli` (optional)
