...
avatar-harness is a Python 3.12+ library with two optional extras:

```bash
pip install avatar-harness[openai]            # the default OpenAI-compatible model client
pip install avatar-harness[openai,textual]    # + the bundled Textual cockpit, `jo-cli` (optional)
```
...
Working from a clone of this repo, `make install` (a thin `uv sync`) sets up
