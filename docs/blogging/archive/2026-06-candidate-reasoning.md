# Archive — blog candidate reasoning (2026-06-15 → 2026-07-09)

> **FROZEN — superseded by [`../blog-candidates.md`](../blog-candidates.md), which is the
> current plan of action.** Do not update this file; do not plan from it.
>
> This is the deliberation that *produced* the plan — the candidate essays, the scoring
> passes, the Codex cross-validation, the rollout ladder — kept for provenance and as raw
> material (the Pi contrast, the differentiator list, the per-post caveats). It was retired
> on 2026-07-12 because iterative edits had left it with four competing "current plans" and
> six overlapping ID schemes for the same ~20 articles.
>
> **Known-stale in here:** every status and date; the `NC1`/`NC2` candidates (both were
> absorbed into post `02`); all relative links (they were written for the parent directory —
> prefix research links with `../../`).

---

## Tech blog candidates — what avatar-harness does differently *(as retired, 2026-07-09)*
