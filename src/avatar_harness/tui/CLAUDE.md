...
nothing under `src/avatar_harness/` outside `tui/` may import from `tui/`. The cockpit owns its
...
- **Behind the `[textual]` extra.** `import avatar_harness` must work without `textual`, so keep
