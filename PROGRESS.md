...
- [ ] **Eval & observability harness** — *scheduled after Phase 3 (REPL); design drafted in **ADR-0004** (Proposed). Prerequisite already landed: provider token-usage capture + `model_usage` event (PR #31).* Make agent performance measurable so changes iterate against data, not vibes:
