...
Design locked in **ADR-0001** (async engine · typed event bus · durable execution) and **ADR-0002** (`docs/adr/0002-interactive-tui-cockpit-and-mvp-feature-set.md` — the MVP coding-agent feature set · **Textual full-screen cockpit** · constrained tier-3 **`run_command`** · **visible modes** over a hidden classifier · **plan mode**), the latter cross-validated with Codex (gpt-5.5/xhigh). Phase 3 is a **layered build**, not a 2.6-style disjoint-file fan-out: a sequential **foundation** (the frozen contract) → a small **contract-first lane team** → a sequential **tail**. Durable execution moves *past* the MVP (ADR-0002 defers crash-resume wiring).

### 3.0 Foundation — the frozen contract ✅ complete 2026-06-09 (16 tests; 155/155 green; `make check` clean)

The single sequential spine the lanes fan out around: the typed event union, the async core, and the two-plane session API. After this lands, no lane edits the event-union *types* or the session API *signatures*.
...
- [x] **Two-plane `Session`** (`session.py`) — `events()` out (cannot block/redirect); `resolve_approval()`/`cancel()` in; an event *announces* an approval need, the control method *decides* it (§13). `EventBus` is the foundation's simple unbounded fan-out.
  - [x] `test_session_events_yields_typed_stream` · `test_session_events_subscriber_cannot_alter_control` · `test_resolve_approval_unblocks_gated_call` · `test_cancel_records_feedback_and_stops` · `test_approval_announced_by_event_not_decided_by_it`

### 3.1 Lanes — contract-first team (pending; tests proposed before code)

- [ ] **Lane 1 · Engine internals** — `AsyncEventBus` (bounded per-subscriber fan-out + drop policy) + privileged write-ahead `JsonlEventJournal`; route `arun()` emissions through `await bus.publish()`. *Depends only on the frozen event types.*
