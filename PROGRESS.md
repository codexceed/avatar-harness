...
- [ ] **3.2b · `@path` grounding + plan mode** — `@file` seeds a file into task context; plan mode = read-only plan task → `PlanModal` approve → seeds the edit task (rides the `investigating→editing` gate).
- [ ] **3.2c · conversational-verification authority + CLI launch** — verify always runs+reports; conversational mode → human is terminal authority, `--auto` keeps the strict §12 gate; plus the CLI flag that launches the cockpit (wires `ReplSession` ↔ `CockpitApp`).
