...
- **Lane 3 · `run_command`** — *Independent.*
  - [x] **The tool** — tier-3 over `Workspace.run` (argv, no shell metacharacters), active in every phase but gate-controlled; default-blocked in batch, approval-gated in the REPL; a ran-but-failed command is `success=True` evidence; timeout is a system failure. Registered in `default_registry`. (6 tests; 164/164 green.)
