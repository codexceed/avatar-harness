...
- [x] **3.2c · plan mode** — a read-only **plan task** (`task_kind="investigate"` + a planning-directive constraint, so mutation stays blocked at the existing phase + `task_kind` gate) proposes a plan; the human **approves or revises**; the approved plan **seeds the edit task as a constraint** (`model_client` surfaces it), which then rides the `investigating→editing` gate. `submit_plan(prompt, decide)` drives the flow — **revise re-runs the plan task** so the model refines it (ADR-0002 D5 mermaid); `start()` in `plan` mode returns the read-only plan session for the cockpit. Plan is a session **mode**, not a `task_kind` (no new control plane); the decision rides a Textual-free `PlanDecision(approved, text)` so the core stays import-light; `/plan` + `/mode plan` enter it. (7 tests; 228/228 green; `make check` clean.)
- [ ] **3.2d · conversational-verification authority** — verify always runs+reports; conversational mode → human is terminal authority, `--auto` keeps the strict §12 gate.
- [ ] **3.2e · CLI launch** — the `--interactive` flag that launches the cockpit (wires `ReplSession` ↔ `CockpitApp`), making the REPL runnable end-to-end.
