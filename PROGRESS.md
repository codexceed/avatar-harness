...
- **Lane 2 · Textual cockpit** — decomposed into three reviewable sub-increments (the cockpit is too big + dependency-heavy for one PR):
  ...
  - [ ] **2b · Textual cockpit shell** — full-screen panes + status bar + input wired to `session.events()`, against a **replay session**; adds the optional `[textual]` extra; `Pilot`/`run_test()` tested.
