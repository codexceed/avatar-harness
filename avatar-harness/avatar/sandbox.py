...
class Bwrap:
    ...
    def prepare(self, argv: list[str], cwd: Path) -> ExecSpec:
        ...
        wrapped = [
            ...
            "/proc",
            "--tmpfs",
            "/tmp",  # noqa: S108 — a guest mount point inside the sandbox, not host temp
            "--bind",
            root,
            root,
            "--chdir",
...
class Container:
    ...
    def prepare(self, argv: list[str], cwd: Path) -> ExecSpec:
        ...
        run = [
            ...
            "--read-only",
            "--tmpfs",
            "/tmp",  # noqa: S108 — a guest mount point inside the container, not host temp
            "--pids-limit",
            str(self._rlimits.pids),
            "-v",
            f"{cwd}:/workspace:rw",
            "-w",
            "/workspace",
